...
class ProjectDetector {
  ...
  async detectLanguages() {
    ...
    for (const detector of detectors) {
      ...
      if (result.confidence > 0.3) { // Only include if reasonable confidence
        results.push(result);
      }
    }
  ...
  async detectPython() {
    ...
    // Python files
    const pythonFiles = await glob('**/*.py', { cwd: this.projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
    ...
    if (fs.existsSync(path.join(this.projectPath, 'src'))) {
      const srcPythonFiles = await glob('src/**/*.py', { cwd: this.projectPath });
    ...
    // ML indicators
    const hasMLFiles = pythonFiles.some(file => 
      file.includes('model') || file.includes('train') || file.includes('predict')
    );
    if (hasMLFiles) {
  ...
  async detectTypeScript() {
    ...
    if (fs.existsSync(path.join(this.projectPath, 'package.json'))) {
      ...
      try {
        const content = JSON.parse(fs.readFileSync(path.join(this.projectPath, 'package.json'), 'utf8'));
    ...
    // TypeScript files
    const tsFiles = await glob('**/*.ts', { cwd: this.projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
    if (tsFiles.length > 0) {
    ...
    const tsxFiles = await glob('**/*.tsx', { cwd: this.projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
    ...
    const jsFiles = await glob('**/*.js', { cwd: this.projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
    ...
    // React indicators
    const hasReact = tsxFiles.length > 0 || 
  ...
  async detectGo() {
    ...
    const goFiles = await glob('**/*.go', { cwd: this.projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
    if (goFiles.length > 0) {
  ...
  async detectRust() {
    ...
    const rustFiles = await glob('**/*.rs', { cwd: this.projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
  ...
  async detectPineScript() {
    ...
    const pineFiles = await glob('**/*.pine', { cwd: this.projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
    ...
    if (pineFiles.length > 0) {
      try {
        ...
        const content = fs.readFileSync(path.join(this.projectPath, sampleFile), 'utf8');
        
        if (content.includes('//@version=')) {
          indicators.push('PineScript version declaration');
          confidence += 0.1;
        }
        
        if (content.includes('indicator(') || content.includes('strategy(')) {
          indicators.push('TradingView indicator/strategy');
          confidence += 0.1;
        }
    ...
    const tradingDirs = ['indicators', 'strategies', 'backtests', 'trading'];
  ...
  async hasMultipleLanguages(threshold = 0.4) {
    const languages = await this.detectLanguages();
    const significantLanguages = languages.filter(lang => lang.confidence >= threshold);
  ...
  async detectPythonProjectType() {
    ...
    const hasFastAPI = await this.checkPythonProjectType(projectPath, 'fastapi', [
      'from fastapi import',
      'import fastapi',
      'FastAPI(',
      'APIRouter'
    ...
    const hasDataScienceLibs = await this.checkPythonProjectType(projectPath, 'data-science', [
      'import pandas',
      'import numpy',
      'import matplotlib',
      'import seaborn',
    ...
    const hasMLLibs = await this.checkPythonProjectType(projectPath, 'machine-learning', [
      'import torch',
      'import tensorflow',
      'import keras',
      'from transformers',
  ...
  async checkPythonProjectType(projectPath, type, patterns) {
    const pythonFiles = await glob('**/*.py', { cwd: projectPath, ignore: ['**/node_modules/**', '**/.git/**'] });
    
    for (const file of pythonFiles.slice(0, 10)) { // Check first 10 files
      try {
