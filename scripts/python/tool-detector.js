    ...
    return {
      ...
      poetry: {
        ...
        installGuide: {
          ...
          windows: 'powershell -c "(Invoke-WebRequest -Uri https://install.python-poetry.org -UseBasicParsing).Content | python -"'
        },
        priority: 7
      },
      pip: {
        ...
        installGuide: {
          macos: 'Usually installed with Python',
  ...
  if (specificTool) {
    detector.detectTool(specificTool).then(result => {
      if (result.installed) {
        ...
        console.log(`   Install: ${detector.getInstallGuide(specificTool) || 'No install guide available'}`);
      }
    });
  } else {
