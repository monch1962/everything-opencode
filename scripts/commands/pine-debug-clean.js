...
class PineScriptDebugger {
  ...
  async inspect(args) {
    ...
    const analysis = await this.analyzeVariables(
      pineFile,
      variables,
      options.bars,
    );
  ...
  async monitor(args) {
    ...
    if (options.condition) {
      ...
      // Analyze condition
      const conditionAnalysis = this.analyzeCondition(
        content,
        options.condition,
      );
  ...
  async profile(args) {
    ...
    if (options.memory || options.metrics.includes("memory")) {
      ...
      console.log(
        `  Estimated peak memory: ${memoryAnalysis.estimatedPeak} units`,
      );
    ...
    if (options.output) {
      ...
      if (options.memory) {
        ...
        console.log(
          `🧠 Memory profiling helpers saved to: ${memoryHelpersPath}`,
        );
      }
    }
  }
  ...
  generateDebugPlotCode(content, variableName) {
...
bgcolor(${variableName} > 0 ? color.new(color.green, 90) : ${variableName} < 0 ? color.new(color.red, 90) : na)`;
  ...
  analyzeComplexity(content) {
    ...
    memoryEstimate = variables * 8 + // Base variable memory
  ...
  analyzeMemoryUsage(content) {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes("array.new_")) {
        ...
        if (line.includes("array.push") && !line.includes("array.shift") && !line.includes("array.pop")) {
          leakRisks.push({
            type: "array_growth",
    ...
    estimatedPeak = variableTypes.size * 8 + // Base variable memory
...
For detailed help on a specific action:
  ...
  generateDebugHelpers(include = "all") {
    return `// PineScript Debugging Helpers
// Generated by /pine-debug helpers
// Include in your PineScript with: //@include "debug-helpers.pine"

// ========== BASIC DEBUGGING ==========

/**
 * Plot a value with debug styling
 */
debug.plot(value, title, color) =>
    plot(value, title, color=color.new(color ? color : color.purple, 0), linewidth=1)

/**
 * Plot a character for discrete values
 */
debug.plotchar(value, title) =>
    plotchar(value, title, "", location.top, size=size.tiny, color=color.new(color.blue, 0))

/**
 * Alert with formatted message
 */
debug.alert(value, title, frequency) =>
    alert(title + ": " + str.tostring(value), frequency ? frequency : alert.freq_once_per_bar)

// ========== SERIES DEBUGGING ==========

/**
 * Debug a series variable with history
 */
debug.series(value, title, lookback) =>
    var debugArray = array.new_float(lookback ? lookback : 20)
    array.push(debugArray, value)
    if array.size(debugArray) > (lookback ? lookback : 20)
        array.shift(debugArray)
    plot(array.get(debugArray, 0), title + " (Current)", color=color.new(color.blue, 0))
    plot(array.get(debugArray, 1), title + " (Prev)", color=color.new(color.blue, 50))

// ========== CONDITION DEBUGGING ==========

/**
 * Monitor a condition with visual feedback
 */
debug.monitor(condition, title) =>
    plotshape(condition, title, shape.triangleup, location.belowbar, color=color.new(color.green, 0), size=size.small)

/**
 * Trace condition changes
 */
debug.trace(condition, title) =>
    var bool lastCondition = na
    if not na(condition)
        if condition != lastCondition
            debug.alert(condition, title + " changed to: ")
        lastCondition := condition

// ========== PERFORMANCE DEBUGGING ==========

/**
 * Measure execution time (conceptual - PineScript doesn't have timers)
 */
debug.profileStart(label) =>
    var int startBar = bar_index
    label.new(bar_index, high, "Profile: " + label + " started", color=color.new(color.gray, 0))

debug.profileEnd(label) =>
    label.new(bar_index, high, "Profile: " + label + " ended", color=color.new(color.gray, 0))
 ...
 */
debug.format(value, precision) =>
    str.tostring(value, precision ? "#." + str.tostring(precision) : "#.##")

/**
 * Create a debug table in data window
 */
debug.table(values, labels) =>
    for i = 0 to array.size(values) - 1
        plotchar(array.get(values, i), array.get(labels, i), "", location.top, size=size.tiny)

// ========== EXAMPLE USAGE ==========
/*
// Example 1: Debug a calculation
myValue = close * 2
debug.plot(myValue, "My Value Debug")

// Example 2: Monitor a condition  
buyCondition = rsi < 30
debug.monitor(buyCondition, "Buy Condition")

// Example 3: Trace variable changes
debug.trace(myValue > 100, "My Value > 100")
*/
`;
  }
}
