...
class ElixirConfigWizard {
  ...
  async detectProjectType() {
    ...
    const projectTypes = [
      {
        id: 'standard',
        name: 'Standard Elixir Application',
    ...
    // Check for umbrella project
      ...
      indicators.some(
        (i) => i.includes('umbrella') || summary.files?.some((f) => f.includes('apps/'))
      )
    ) {
    ...
    // Check for library
      ...
      summary.files?.some((f) => f.includes('mix.exs')) &&
  ...
  async configureTools(detectedTools) {
    ...
    if (exDocInstalled) {
      ...
      const installExDoc = await this.prompts.confirm(
        'Install ex_doc for documentation generation?',
        true
      );
    ...
    if (excoverallsInstalled) {
      ...
      const installCoverage = await this.prompts.confirm(
        'Install excoveralls for test coverage?',
        false
      );
  ...
  async configureLinting(detectedTools) {
    ...
    if (credoInstalled) {
      ...
      const installCredo = await this.prompts.confirm(
        'Install Credo for static code analysis?',
        true
      );
      if (installCredo) {
        lintingConfig.tools.push('credo');
    ...
    if (dialyzerInstalled) {
      ...
      const installDialyzer = await this.prompts.confirm(
        'Enable Dialyzer for success typing analysis?',
        false
      );
      if (installDialyzer) {
        lintingConfig.tools.push('dialyzer');
  ...
  async configureTesting(projectType) {
    ...
    if (projectType === 'phoenix') {
      ...
      testingConfig.browser = await this.prompts.confirm(
        'Enable browser testing with Wallaby/Hound?',
        false
      );
    ...
    const enablePropertyTesting = await this.prompts.confirm(
      'Enable property-based testing with StreamData?',
      false
    );
  ...
  async configureSecurity(detectedTools) {
    ...
    if (sobelowInstalled) {
      ...
      const installSobelow = await this.prompts.confirm(
        'Install Sobelow for security-focused static analysis?',
        true
      );
      if (installSobelow) {
        securityConfig.tools.push('sobelow');
    ...
    if (mixAuditInstalled) {
      ...
      const installMixAudit = await this.prompts.confirm(
        'Install mix_audit for dependency security auditing?',
        true
      );
      if (installMixAudit) {
        securityConfig.tools.push('mix_audit');
  ...
  async configurePhoenix() {
    ...
    phoenixConfig.authentication = await this.prompts.confirm(
  ...
  async configureUmbrella() {
    ...
    const appCount = await this.prompts.number('How many applications in the umbrella?', 2, 1, 20);
    ...
    umbrellaConfig.sharedDependencies = await this.prompts.confirm(
  ...
  line_length: 80
    ...
    if (userConfig.projectType === 'umbrella') {
      const subdirs = userConfig.umbrella?.applications?.map((app) => app.name).join(', ') || '';
      formatterContent = `[
  import_deps: [:ecto, :phoenix],
  inputs: ["*/{config,lib,test}/**/*.{ex,exs}", "*.exs"],
  line_length: 80,
  ...
  /**
   * Quick setup with automatic detection and defaults
   */
  async quickSetup() {
    try {
      this.prompts.header('⚡ Quick Setup for Elixir Project');
      ...
      const defaultConfig = {
        projectType,
        elixir: {
          version: detectedTools.elixirInfo?.elixirVersion || '1.19',
          erlangVersion: detectedTools.erlangInfo?.otpVersion || '26',
        },
        tools: {
          packageManager: 'hex',
          buildTool: 'mix',
          documentation: detectedTools.ex_doc?.installed ? 'ex_doc' : 'none',
          coverage: detectedTools.excoveralls?.installed ? 'excoveralls' : 'none',
        },
        linting: {
          enabled: true,
          tools: detectedTools.credo?.installed ? ['credo'] : [],
        },
        testing: {
          enabled: true,
          framework: 'exunit',
        ...
        formatting: {
          ...
          checkInCI: true,
          autoFormat: true,
          rules: ['imports', 'aliases'],
        },
        security: {
          enabled: true,
          tools: detectedTools.sobelow?.installed ? ['sobelow'] : [],
      ...
      if (projectType === 'phoenix') {
        defaultConfig.phoenix = {
          version: '1.7',
          database: 'postgres',
          liveView: true,
          liveViewVersion: '0.20',
          authentication: false,
        };
      } else if (projectType === 'umbrella') {
        defaultConfig.umbrella = {
          applications: [
            { name: 'app_1', type: 'standard' },
            { name: 'app_2', type: 'standard' },
          ],
      ...
      // Save configuration
      const saved = await this.saveConfiguration(defaultConfig);

      if (saved) {
        this.prompts.success('Quick setup completed!');
        this.prompts.info('Run /elixir-setup without --quick for interactive configuration.');
      }
